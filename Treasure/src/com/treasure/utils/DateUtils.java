...
public class DateUtils {
	 ...
	 * @return int
	 */
	public static int compareTime(Date begin, Date end) {
		int value = 0;
		try {
			long between = (end.getTime() - begin.getTime()) / 1000; // 除以1000转换成豪秒
			value = (int) (between % (24 * 36000) / 3600);
	 ...
	 * @return 日期数组
	 */
	public static List<String> pastDate(int intervals, String date, String type) {
		List<String> pastDaysList = new ArrayList<>();
		for (int i = intervals; i > 0; i--) {
			pastDaysList.add(getPastDate(i, date, type));
		}
		return pastDaysList;
	}
	 ...
	 * @return 日期数组
	 */
	public static List<String> pastWeek(int intervals, String date, String separator, String type) {
		List<String> pastDaysList = new ArrayList<String>();
		for (int i = intervals; i > 0; i--) {
			pastDaysList.add(getPastDate(i * 7 - 1, date, type) + separator + getPastDate((i - 1) * 7, date, type));
		}
		return pastDaysList;
	}
	 ...
	 * @return 日期数组
	 */
	public static List<String> fetureDate(int intervals, String date) {
	 ...
	 * @return
	 */
	public static String getPastDate(int past, String date, String type) {
		Calendar calendar = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static String getFetureDate(int past, String date) {
		Calendar calendar = Calendar.getInstance();
		if (StringUtils.isNotBlank(date)) {
			try {
				calendar.setTime(new SimpleDateFormat("yyyy-MM-dd").parse(date));
	...
	public static String getLastWeekFirstDate(String format) {
		...
		int first_day_of_week = now.get(Calendar.DATE) + 2 - today; // 星期一
		now.set(now.DATE, first_day_of_week);
		Calendar calendar = getCalendar(now.getTime(), format);
		SimpleDateFormat sdf = DateFormatUtils.getFormat(format);
	 ...
	 */
	@SuppressWarnings("static-access")
	public static String geLastWeektLastDate(String format) {
		Calendar now = Calendar.getInstance();
		now.setTime(new Date());
		int today = now.get(Calendar.DAY_OF_WEEK);
		int first_day_of_week = now.get(Calendar.DATE) + 2 - today; // 星期一
	 ...
	 */
	@SuppressWarnings("static-access")
	public static String geNextWeektLastDate(String format) {
		Calendar now = Calendar.getInstance();
		now.setTime(new Date());
		int today = now.get(Calendar.DAY_OF_WEEK);
		int first_day_of_week = now.get(Calendar.DATE) + 2 - today; // 星期一
	...
	public static String parseTZDate(String time, String format) {
		try {
	...
	public static void main(String[] args) {
		System.out.println(pastWeek(3, "2017-05-07", "/", "yyyy-MM-dd"));
	}
}
