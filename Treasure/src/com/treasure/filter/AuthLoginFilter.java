...
public class AuthLoginFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws IOException, ServletException {
		...
		if (isNotFilter(requestURL)) {
			chain.doFilter(request, resp);
			return;
		}
		if (requestURL.contains("mgr")) {// 后台请求
			if (session.getAttribute(SysConstants.LOGIN_KEY) != null) {
				chain.doFilter(request, resp);
				return;
			} else {
				// 重定向到登录页面
				String path = request.getContextPath() + "/mgr/user/tologin";
